...
public class Snake
{
    ...
    /**
     * Añade un método llamado mover a la clase Snake que simule el movimiento de la misma. Para ello debes eliminar el segmento que esta en la cola de la serpiente 
     * y añadir un segmento aleatorio en el inicio de la misma. Este método no dibuja ni borra nada en la pantalla, solo quita un segmento y pone otro. 
     * Evidentemente, el segmento añadido en la cabeza no debe colisionar ni con los bordes del lienzo ni con el resto de segmentos de la serpiente. Si no es posible 
    ...
    public int getXCabeza(){
        Segment ultimoSegmento = segmentos.get(segmentos.size() - 1);
        return ultimoSegmento.getPosicionInicialX();
    }
    
        public int getYCabeza(){
        Segment ultimoSegmento = segmentos.get(segmentos.size() - 1);
        return ultimoSegmento.getPosicionInicialY();
    }
}
